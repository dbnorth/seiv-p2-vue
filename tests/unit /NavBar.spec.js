...
describe('NavBar Test', () => {
    ...
    test('if a user is Admin show corret Menu Options', async () => {
        ...
        expect(wrapper.findComponent({ ref: 'advisor-edit' }).exists()).toBe(
            true,
        );
        expect(wrapper.findComponent({ ref: 'student-list' }).exists()).toBe(
            true,
        );
        expect(wrapper.findComponent({ ref: 'advisor-list' }).exists()).toBe(
            true,
        );
        expect(wrapper.findComponent({ ref: 'course-list' }).exists()).toBe(
            true,
        );
        expect(wrapper.findComponent({ ref: 'semester-list' }).exists()).toBe(
            true,
        );
        expect(wrapper.findComponent({ ref: 'degree-list' }).exists()).toBe(
            true,
        );
    });
    ...
    test('if a user is Student show corret Menu Options', async () => {
        ...
        expect(wrapper.findComponent({ ref: 'home' }).exists()).toBe(true);
        expect(wrapper.findComponent({ ref: 'logout' }).exists()).toBe(true);
        expect(wrapper.findComponent({ ref: 'student-edit' }).exists()).toBe(
            true,
        );
        expect(
            wrapper.findComponent({ ref: 'studentcourse-list' }).exists(),
        ).toBe(true);
        expect(wrapper.findComponent({ ref: 'student-list' }).exists()).toBe(
            false,
        );
